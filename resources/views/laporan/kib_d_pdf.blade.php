...
<body style="font-size: {{ $fontSize }};">
    ...
    <div style="text-align: center; margin-bottom: 15px;">
        <h4 style="margin: 0; text-transform: uppercase;">KARTU INVENTARIS BARANG (KIB) D</h4>
        <h5 style="margin: 0; font-weight: normal;">JALAN, IRIGASI DAN JARINGAN</h5>
    </div>
    ...
    <div style="margin-bottom: 15px;">
        <table style="width: 100%; border: none !important; table-layout: auto !important;">
            ...
            <tr>
                ...
                <td style="border: none !important; padding: 1px 0; text-align: center;">:</td>
                <td style="border: none !important; padding: 1px 0; text-align: left;">{{ str_replace('XX', '04', $profile->kode_lokasi_bmd) . '.' . date('Y') }}</td>
            </tr>
        </table>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="80" rowspan="2">Konstruksi</th>
                <th width="30" rowspan="2">Pan jang (m)</th>
                <th width="30" rowspan="2">Lebar (m)</th>
                <th width="40" rowspan="2">Luas (M2)</th>
                <th width="100" rowspan="2">Letak / Lokasi</th>
                <th colspan="2">Dokumen Jalan</th>
                <th width="40" rowspan="2">Status Tanah</th>
                <th width="50" rowspan="2">No. Kode Tanah</th>
        ...
        <tbody>
            ...
            <tr>
                ...
                <td>{{ $item->kibD->konstruksi ?? '-' }}</td>
                <td class="text-center">{{ $item->kibD->panjang ?? '-' }}</td>
                <td class="text-center">{{ $item->kibD->lebar ?? '-' }}</td>
                <td class="text-center">{{ $item->kibD->luas ?? '-' }}</td>
                <td>{{ $item->kibD->letak_lokasi ?? '-' }}</td>
                <td class="text-center">{{ $item->kibD->dokumen_tanggal ?? '-' }}</td>
                <td>{{ $item->kibD->dokumen_nomor ?? '-' }}</td>
                <td class="text-center">{{ $item->kibD->status_tanah ?? '-' }}</td>
