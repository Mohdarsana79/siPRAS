...
<body style="font-size: {{ $fontSize }};">
    ...
    <div style="text-align: center; margin-bottom: 15px;">
        <h4 style="margin: 0; text-transform: uppercase;">KARTU INVENTARIS BARANG (KIB) B</h4>
        <h5 style="margin: 0; font-weight: normal;">PERALATAN DAN MESIN</h5>
    </div>
    ...
    <div style="margin-bottom: 15px;">
        <table style="width: 100%; border: none !important; table-layout: auto !important;">
            ...
            <tr>
                ...
                <td style="border: none !important; padding: 1px 0; text-align: center;">:</td>
                <td style="border: none !important; padding: 1px 0; text-align: left;">{{ str_replace('XX', '02', $profile->kode_lokasi_bmd) . '.' . date('Y') }}</td>
            </tr>
        </table>
    </div>
    ...
    <table>
        <thead>
            <tr>
                ...
                <th width="60" rowspan="2">Kode Barang</th>
                <th width="80" rowspan="2">Nama/Jenis Barang</th>
                <th width="20" rowspan="2">Reg.</th>
                <th width="60" rowspan="2">Merk / Type</th>
                <th width="40" rowspan="2">Ukuran / CC</th>
                <th width="40" rowspan="2">Bahan</th>
                <th width="30" rowspan="2">Tahun Beli</th>
            ...
            <tr>
                <th width="40">Pabrik</th>
                <th width="50">Rangka</th>
                <th width="50">Mesin</th>
                <th width="40">Polisi</th>
                <th width="40">BPKB</th>
            </tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td class="text-center">{{ $item->kibB->tahun_pembuatan ?? '-' }}</td>
                <td>{{ $item->kibB->nomor_pabrik ?? '-' }}</td>
                <td>{{ $item->kibB->nomor_rangka ?? '-' }}</td>
                <td>{{ $item->kibB->nomor_mesin ?? '-' }}</td>
                <td class="text-center">{{ $item->kibB->nomor_polisi ?? '-' }}</td>
                <td>{{ $item->kibB->nomor_bpkb ?? '-' }}</td>
