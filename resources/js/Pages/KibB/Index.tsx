...
export default function Index({ items, kategoris, ruangans, sumberDanas, filters }: { items: PaginatedData<Item>, kategoris: Kategori[], ruangans: Ruangan[], sumberDanas: SumberDana[], filters: { search?: string } }) {
    ...
    useEffect(() => {
        const delayDebounceFn = setTimeout(() => {
            if (searchTerm !== (filters.search || '')) {
                router.get(route('kib-b.index'), { search: searchTerm }, {
                    preserveState: true,
                    replace: true
                });
            }
    ...
    const openModal = (item?: Item) => {
        ...
        if (item) {
            ...
            setData({
                ...
                merk_tipe: item.kib_b?.merk_tipe || '',
                ukuran_cc: item.kib_b?.ukuran_cc || '',
                bahan: item.kib_b?.bahan || '',
                tahun_pembuatan: item.kib_b?.tahun_pembuatan || new Date().getFullYear().toString(),
                nomor_pabrik: item.kib_b?.nomor_pabrik || '',
                nomor_rangka: item.kib_b?.nomor_rangka || '',
                nomor_mesin: item.kib_b?.nomor_mesin || '',
                nomor_polisi: item.kib_b?.nomor_polisi || '',
                nomor_bpkb: item.kib_b?.nomor_bpkb || '',
            });
        } else {
            setIsEditing(false);
            setEditingId(null);
    ...
    const submit = (e: React.FormEvent) => {
        ...
        if (isEditing && editingId) {
            put(route('kib-b.update', editingId), {
                onSuccess: () => closeModal(),
            });
        } else {
            post(route('kib-b.store'), {
                onSuccess: () => closeModal(),
            });
        }
    };
    ...
    const handleDelete = () => {
        if (selectedId) {
            destroy(route('kib-b.destroy', selectedId), {
        ...
        <AuthenticatedLayout
            header={
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h2 className="text-2xl font-bold text-gray-900 tracking-tight text-transparent bg-clip-text bg-gradient-to-r from-amber-600 to-orange-700">KIB B - Peralatan & Mesin</h2>
                    </div>
                </div>
            }
        >
            <Head title="KIB B - Peralatan & Mesin" />
                ...
                <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden flex flex-col">
                    ...
                    <div className="p-4 sm:p-5 border-b border-gray-100 flex flex-col md:flex-row md:items-center justify-between gap-4 bg-gray-50/30">
                        <div className="relative w-full md:max-w-md group">
                            <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-gray-400 group-focus-within:text-amber-500">
                                <Icons.Search className="w-4 h-4" />
                            </div>
                            <TextInput
                                type="text"
                                className="block w-full pl-10 pr-4 py-2.5 text-sm rounded-xl border-gray-200 bg-white focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all font-medium"
                        ...
                        <div className="flex items-center gap-3">
                            <PrimaryButton
                                onClick={() => openModal()}
                                className="!rounded-xl !py-2.5 px-5 flex items-center gap-2 !bg-amber-600 hover:!bg-amber-700 shadow-sm transition-all justify-center whitespace-nowrap !text-xs"
                            >
                        ...
                        <table className="w-full text-sm text-left border-collapse">
                            <thead className="bg-gradient-to-r from-indigo-600 to-violet-600">
                                <tr>
                                    ...
                                    <th className="px-6 py-4 font-black text-white/90 uppercase tracking-widest text-[10px]">Spesifikasi</th>
                                    <th className="px-6 py-4 font-black text-white/90 uppercase tracking-widest text-[10px]">Identitas</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {items.data.map((item) => (
                                    <tr key={item.id} className="group hover:bg-amber-50/30 transition-all duration-200">
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col">
                                                <span className="font-bold text-gray-900 group-hover:text-amber-600 transition-colors uppercase tracking-tight leading-tight">{item.nama_barang}</span>
                                                <div className="flex items-center gap-2 mt-1">
                                                    ...
                                                    <span className="text-[10px] font-black text-amber-500 uppercase tracking-tighter">Reg: {item.nomor_register}</span>
                                                </div>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col">
                                                <span className="font-black text-orange-600 text-xs">{item.kib_b?.merk_tipe}</span>
                                                <span className="text-[10px] text-gray-500 mt-1 uppercase truncate max-w-[150px]">Bahan: {item.kib_b?.bahan} | Thn: {item.kib_b?.tahun_pembuatan}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col">
                                                <span className="text-[11px] font-bold text-gray-800">{item.kib_b?.nomor_polisi || '-'}</span>
                                                <span className="text-[10px] text-gray-400 mt-0.5">SN/Msn: {item.kib_b?.nomor_mesin || '-'}</span>
                                            </div>
                                        </td>
                                        <td className="px-6 py-5">
                                            <div className="flex flex-col">
                                        ...
                                        <td colSpan={5} className="px-6 py-20 text-center">
                                            <div className="flex flex-col items-center gap-3">
                                                <div className="p-4 bg-gray-50 rounded-full">
                                                    <Icons.Package className="w-10 h-10 text-gray-300" />
                                                </div>
                                                <p className="text-sm font-bold text-gray-400 uppercase tracking-widest">Tidak ada data peralatan ditemukan</p>
                                            </div>
                                        </td>
                                    </tr>
                                )}
                    ...
                    <div className="p-4 sm:p-6 bg-white border-t border-gray-100 flex flex-col justify-between items-center gap-4">
                        <div className="flex flex-col sm:flex-row justify-between items-center w-full gap-4">
                            <div className="text-sm font-medium text-gray-500">
                                Menampilkan <span className="text-gray-900 font-black">{items.data.length}</span> dari <span className="text-gray-900 font-black">{items.total}</span> Aset Peralatan & Mesin
                            </div>
            ...
            <Modal show={isModalOpen} onClose={closeModal} maxWidth="6xl">
                <div className="bg-white/95 backdrop-blur-xl max-h-[90vh] overflow-y-auto custom-scrollbar rounded-[2.5rem]">
                    {/* Artistic Header */}
                    <div className={`px-8 pt-10 pb-12 relative flex flex-col items-center text-center overflow-hidden ${isEditing ? 'bg-gradient-to-br from-amber-500 via-orange-600 to-amber-700' : 'bg-gradient-to-br from-orange-500 via-amber-500 to-yellow-600'}`}>
                        ...
                        <h3 className="text-2xl font-black text-white tracking-tight relative z-10 uppercase">
                            {isEditing ? 'UPDATE PERALATAN & MESIN' : 'TAMBAH PERALATAN & MESIN'}
                        </h3>
                        <p className="text-white/80 text-[10px] font-bold uppercase tracking-[0.3em] mt-2 relative z-10">
                            Pencatatan Kartu Inventaris Barang - KIB B
                        </p>
                    </div>
                    ...
                    <form onSubmit={submit} className="px-8 pb-8 -mt-6 bg-white rounded-t-[2.5rem] relative z-20 pt-8">
                        <div className="grid grid-cols-1 md:grid-cols-12 gap-x-12 gap-y-5">
                            ...
                            <div className="space-y-4 md:col-span-6">
                                <h4 className="flex items-center gap-2 text-[10px] font-black text-orange-500 uppercase tracking-[0.2em] mb-4">
                                    <div className="w-6 h-px bg-orange-100"></div> Informasi Utama
                                </h4>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="group/field relative">
                                        ...
                                        <TextInput id="kode_barang" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all text-sm uppercase tracking-widest" value={data.kode_barang} onChange={(e) => setData('kode_barang', e.target.value)} required placeholder="KODE..." />
                                    ...
                                    <div className="group/field relative">
                                        ...
                                        <TextInput id="nomor_register" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all text-sm uppercase tracking-widest" value={data.nomor_register} onChange={(e) => setData('nomor_register', e.target.value)} required placeholder="REG..." />
                                ...
                                <div className="group/field relative">
                                    ...
                                    <TextInput id="nama_barang" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all text-sm" value={data.nama_barang} onChange={(e) => setData('nama_barang', e.target.value)} required placeholder="CONTOH: LAPTOP DELL XPS 13..." />
                                ...
                                <div className="group/field relative">
                                    ...
                                    <TextInput
                                        id="harga"
                                        type="text"
                                        className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-orange-500/10 focus:border-orange-500 transition-all text-sm uppercase placeholder:text-gray-300 tracking-widest"
                                ...
                                <div className="group/field relative">
                                    ...
                                    <TextInput id="asal_usul" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-blue-500/10 focus:border-blue-500 transition-all text-sm uppercase tracking-widest" value={data.asal_usul} onChange={(e) => setData('asal_usul', e.target.value)} required placeholder="MISAL: BOSP 2026" />
                            ...
                            <div className="space-y-4 md:col-span-6">
                                <h4 className="flex items-center gap-2 text-[10px] font-black text-amber-500 uppercase tracking-[0.2em] mb-4">
                                    <div className="w-6 h-px bg-amber-100"></div> Spesifikasi KIB B
                                </h4>

                                <div className="group/field relative">
                                    <InputLabel htmlFor="merk_tipe" value="MERK / TIPE" className="mb-1.5 ml-1 text-[10px] font-black text-gray-400 tracking-widest" />
                                    <TextInput id="merk_tipe" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm uppercase" value={data.merk_tipe} onChange={(e) => setData('merk_tipe', e.target.value)} required placeholder="CONTOH: HONDA VARIO 150..." />
                                    <InputError message={errors.merk_tipe} className="mt-2 text-[10px] font-bold" />
                                </div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div className="group/field relative">
                                        <InputLabel htmlFor="bahan" value="BAHAN" className="mb-1.5 ml-1 text-[10px] font-black text-gray-400 tracking-widest" />
                                        <TextInput id="bahan" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm uppercase" value={data.bahan} onChange={(e) => setData('bahan', e.target.value)} required placeholder="BESI / PLASTIK..." />
                                        <InputError message={errors.bahan} className="mt-2 text-[10px] font-bold" />
                                    </div>
                                    <div className="group/field relative">
                                        <InputLabel htmlFor="tahun_pembuatan" value="THN BUAT" className="mb-1.5 ml-1 text-[10px] font-black text-gray-400 tracking-widest" />
                                        <TextInput id="tahun_pembuatan" type="number" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm tracking-widest" value={data.tahun_pembuatan} onChange={(e) => setData('tahun_pembuatan', e.target.value)} required />
                                        <InputError message={errors.tahun_pembuatan} className="mt-2 text-[10px] font-bold" />
                                    </div>
                                </div>

                                <div className="grid grid-cols-2 gap-4">
                                    <div className="group/field relative">
                                        <InputLabel htmlFor="nomor_polisi" value="NO. POLISI" className="mb-1.5 ml-1 text-[10px] font-black text-gray-400 tracking-widest" />
                                        <TextInput id="nomor_polisi" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm uppercase" value={data.nomor_polisi} onChange={(e) => setData('nomor_polisi', e.target.value)} placeholder="B 1234 ABC" />
                                    </div>
                                    <div className="group/field relative">
                                        <InputLabel htmlFor="nomor_bpkb" value="NO. BPKB" className="mb-1.5 ml-1 text-[10px] font-black text-gray-400 tracking-widest" />
                                        <TextInput id="nomor_bpkb" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm uppercase" value={data.nomor_bpkb} onChange={(e) => setData('nomor_bpkb', e.target.value)} placeholder="NO..." />
                                    </div>
                                </div>

                                <div className="group/field relative">
                                    <InputLabel htmlFor="nomor_mesin" value="NOMOR MESIN / SERI" className="mb-1.5 ml-1 text-[10px] font-black text-gray-400 tracking-widest" />
                                    <TextInput id="nomor_mesin" className="w-full px-4 rounded-xl border-gray-100 bg-gray-50 py-2.5 font-black text-gray-800 focus:ring-4 focus:ring-amber-500/10 focus:border-amber-500 transition-all text-sm uppercase tracking-widest" value={data.nomor_mesin} onChange={(e) => setData('nomor_mesin', e.target.value)} placeholder="MSN / SN..." />
                                </div>
                        ...
                        <div className="flex items-center gap-4 mt-12">
                            ...
                            <PrimaryButton
                                className={`flex-[1.5] !rounded-[1.2rem] !py-3.5 shadow-xl border-none justify-center ${isEditing ? '!bg-gradient-to-r from-amber-600 to-orange-700 shadow-amber-100' : '!bg-gradient-to-r from-orange-600 to-amber-700 shadow-orange-100'} hover:-translate-y-1 active:translate-y-0 transition-all`}
                                disabled={processing}
                            >
            ...
            <Modal show={isDetailModalOpen} onClose={closeDetailModal} maxWidth="3xl">
                <div className="bg-white/95 backdrop-blur-xl max-h-[90vh] overflow-y-auto custom-scrollbar rounded-[2.5rem]">
                    <div className="px-8 pt-10 pb-12 relative flex flex-col items-center text-center overflow-hidden bg-gradient-to-br from-amber-700 via-orange-600 to-amber-700">
                        <div className="absolute top-0 right-0 -mr-10 -mt-10 w-48 h-48 bg-white/10 rounded-full blur-3xl"></div>
                        ...
                        <h3 className="text-xl font-black text-white tracking-tight uppercase">Detail Peralatan & Mesin</h3>
                        ...
                        {selectedItemForDetail && (
                            <div className="space-y-8">
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                    ...
                                    <div className="space-y-4">
                                        <h4 className="text-[10px] font-black text-gray-400 uppercase tracking-widest border-b border-gray-100 pb-2">KIB B - Spesifikasi</h4>
                                        <div className="space-y-3">
                                            {[
                                                { label: 'Merk / Tipe', value: selectedItemForDetail.kib_b?.merk_tipe },
                                                { label: 'Bahan / Thn', value: `${selectedItemForDetail.kib_b?.bahan} / ${selectedItemForDetail.kib_b?.tahun_pembuatan}` },
                                                { label: 'CC / Kapasitas', value: selectedItemForDetail.kib_b?.ukuran_cc || '-' },
                                                { label: 'No. Polisi', value: selectedItemForDetail.kib_b?.nomor_polisi || '-' },
                                                { label: 'No. Mesin / SN', value: selectedItemForDetail.kib_b?.nomor_mesin || '-' },
                                            ].map((d, i) => (
                                                <div key={i} className="flex flex-col">
                                                    <span className="text-[9px] font-bold text-gray-400 uppercase tracking-tighter">{d.label}</span>
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 pt-6 border-t border-gray-50">
                                    {[
                                        ...
                                        { label: 'Nilai Harga', value: formatRupiah(selectedItemForDetail.harga), color: 'text-orange-600' },
                                    ].map((d, i) => (
